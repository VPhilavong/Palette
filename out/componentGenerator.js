...
class ComponentGenerator {
    ...
    async generateWithIntelligentContext(prompt, workspaceInfo) {
        try {
            // Get intelligent context with timeout protection
            const { CodebaseAnalyzer } = await Promise.resolve().then(() => __importStar(require('./codebaseAnalyzer')));
            ...
            const intelligentContext = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Intelligent context timeout')), 10000))
            ]);
    ...
    buildEnhancedSystemPrompt(workspaceInfo, intelligentContext) {
        ...
        if (intelligentContext.relevantComponents && intelligentContext.relevantComponents.length > 0) {
            ...
            intelligentContext.relevantComponents.slice(0, 3).forEach((comp, index) => {
                ...
                if (comp.ast?.hooks && comp.ast.hooks.length > 0) {
                    enhancedPrompt += `   - Hooks: ${comp.ast.hooks.map((h) => h.name).join(', ')}\n`;
                }
                if (comp.ast?.styling) {
        ...
        if (intelligentContext.codeExamples && intelligentContext.codeExamples.length > 0) {
            ...
            intelligentContext.codeExamples.slice(0, 2).forEach((example, index) => {
        ...
        if (intelligentContext.patternSuggestions && intelligentContext.patternSuggestions.length > 0) {
            ...
            intelligentContext.patternSuggestions.slice(0, 4).forEach((pattern, index) => {
    ...
    buildDesignSystemContext(designSystem) {
        let context = '\n🎨 DESIGN SYSTEM:\n';
    ...
    buildStateManagementContext(stateContext) {
        let context = '\n📊 STATE MANAGEMENT:\n';
        ...
        if (stateContext.patterns && stateContext.patterns.length > 0) {
            ...
            context += `- Common Patterns: ${topPatterns.map((p) => p.name).join(', ')}\n`;
        }
        return context;
    }
