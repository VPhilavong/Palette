...
from ..openai_integration.structured_output import (
...
class ZeroFixPipeline:
    ...
    async def process(
        ...
        try:
            ...
            # Stage 0: Component type validation (if prompt provided)
            if prompt:
                self._print_stage_progress(0, "Component type validation")
                type_valid, type_issues = (
                    self.traditional_validator.validate_component_type(
                        current_code, prompt
                    )
                )
                if not type_valid:
                    print(f"   ❌ Component type mismatch: {type_issues}")
                    # Force a regeneration if component type is completely wrong
                    if any("instead of" in issue for issue in type_issues):
                        return {
                            "success": False,
                            "confidence_score": 0.0,
                            "component_code": current_code,
                            "issues": {"errors": type_issues},
                            "error": "Component type mismatch - wrong component generated",
                        }

            # Stage 1: Initial OpenAI validation
            self._print_stage_progress(1, "OpenAI Code Interpreter validation")
            initial_validation = await self.openai_assistant.validate_with_interpreter(
                current_code
            )
            validation_reports.append(
                {"stage": "openai_initial", "result": initial_validation}
            )
            ...
            while iteration < self.max_iterations:
                ...
                has_errors = (
                    initial_validation.get("has_errors", False)
                    if iteration == 1
                    else validation_result.get("has_errors", False)
                )
                ...
                fixed_code, fix_metadata = await self._fix_with_structured_output(
                    current_code, validation_reports[-1]["result"], context
                )
                ...
                if fixed_code != current_code:
                    ...
                    validation_result = (
                        await self.openai_assistant.validate_with_interpreter(
                            current_code
                        )
                    )
                    validation_reports.append(
                        {
                            "stage": f"openai_iteration_{iteration}",
            ...
            if self.enable_mcp_validation and self.mcp_client:
                ...
                if not mcp_result.get("compliant", True):
                    ...
                    current_code = await self._apply_mcp_fixes(
                        current_code, mcp_result, context
                    )

                    # Re-validate with MCP
                    mcp_revalidation = await self._validate_with_mcp(
            ...
            if self.enable_real_project_tests:
                ...
                real_validation = await self._validate_in_real_project(
                    current_code, target_path or "Component.tsx"
                )
                validation_reports.append(
                    {"stage": "real_project", "result": real_validation}
                )

                if not real_validation.get("success", False):
                    print("   🛠️ Fixing real project issues...")
                    current_code = await self._fix_real_project_issues(
                        current_code, real_validation, context
                    )

                    # Re-validate
                    final_real_validation = await self._validate_in_real_project(
                        current_code, target_path or "Component.tsx"
                    )
                    validation_reports.append(
                        {"stage": "real_project_final", "result": final_real_validation}
                    )

            # Stage 5: Final AI Polish
            self._print_stage_progress(5, "Final AI Polish")
            final_code = await self._apply_final_polish(
                current_code, validation_reports, mcp_validations, context
            )

            # Final validation
            final_validation = await self.openai_assistant.validate_with_interpreter(
                final_code
            )
            ...
            confidence_score = self._calculate_confidence_score(
                validation_reports, mcp_validations
            )

            success = (
                final_issues == 0 and confidence_score >= self.confidence_threshold
            )

            self._print_completion_summary(
                original_issues, final_issues, confidence_score, success
            )
    ...
    async def _fix_with_structured_output(
        self, code: str, validation_result: Dict[str, Any], context: Dict[str, Any]
    ) -> Tuple[str, str]:
        ...
        try:
            ...
            return (
                fixed_component.component_code,
                f"Fixed {len(errors)} errors, {len(warnings)} warnings",
            )
        except Exception as e:
    ...
    async def _apply_mcp_fixes(
        self, code: str, mcp_result: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
    ...
    async def _validate_in_real_project(
        ...
        try:
            ...
            ts_result = await self.function_caller.execute_function(
                "validate_typescript", {"code": code, "file_name": file_name}
            )
            validation_results["typescript"] = ts_result

            # ESLint validation
            eslint_result = await self.function_caller.execute_function(
                "run_linter", {"code": code, "file_name": file_name, "fix": True}
            )
            ...
            ts_valid = ts_result.get("success") and ts_result.get("result", {}).get(
                "valid", False
            )
            eslint_success = eslint_result.get("success") and eslint_result.get(
                "result", {}
            ).get("success", False)
            imports_valid = len(import_issues) == 0

            validation_results["overall_success"] = (
                ts_valid and eslint_success and imports_valid
            )
    ...
    async def _validate_imports(self, code: str) -> List[Dict[str, Any]]:
        ...
        for import_path in imports:
            ...
            result = await self.function_caller.execute_function(
                "check_import_availability", {"import_path": import_path}
            )
    ...
    def _calculate_confidence_score(
        ...
        if validation_reports:
            ...
            final_errors = (
                len(final_report.get("result", {}).get("errors", []))
                if final_report
                else 0
            )
        ...
        real_validation = next(
            (
                ...
                if r.get("stage") in ["real_project_final", "real_project"]
            ),
            None,
        )
    ...
    def _print_completion_summary(
        ...
        if original_issues > 0:
            ...
            print(
                f"📉 Issues: {original_issues} → {final_issues} (-{issues_fixed}, {reduction_percent:.1f}% reduction)"
            )
        else:
            print(f"📉 Issues: {original_issues} → {final_issues}")

        # Confidence score with visual indicator
        confidence_bar = "█" * int(confidence_score * 20) + "░" * (
            20 - int(confidence_score * 20)
        )
    ...
    def get_pipeline_stats(self, results: List[ZeroFixResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_issues_fixed": sum(
                r.original_issues - r.final_issues for r in results
            ),
